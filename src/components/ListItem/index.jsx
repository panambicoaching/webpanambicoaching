...
const cardInfo = {
    ...
    physical_therapy: { text: "Es lo que consideramos la parte visible y todo lo que conforma su interior. Es nuestro «templo».", title: "Físico" },
    mood: { text: "No lo podemos ver, pero sí sentirlo. Representa nuestras experiencias en el mundo y cómo lo interpretamos.", title: "Emocional" },
...
const ListItem = ({ design, name, cardsAreLinks }) => {
            ...
            <Link className="card-link" to={`/${name}`}>
                <Card className={design}>
                    ...
                    <Card.Body className={design}>
                        <Card.Title className="text-title">{cardInfo[name].title}</Card.Title>
                        <Card.Text className="text-body">{cardInfo[name].text}</Card.Text>
                    </Card.Body>
                </Card>
            </Link>
        );
    ...
    return (
        <Card className={`${design}`}>
            ...
            <Card.Body>
                <Card.Title className="text-title">{cardInfo[name].title}</Card.Title>
                <Card.Text className={`${design === "transparent" ? "text-body" : "text-body2"}`}>{cardInfo[name].text}</Card.Text>
            </Card.Body>
        </Card>
    );
};
